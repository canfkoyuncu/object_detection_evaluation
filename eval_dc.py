...
def eval(computed, gold):
    ...
    for i in range(1, nCompLbls):
        ccComp = computed==i
        for j in range(1, nGoldLbls):
...
if __name__ == "__main__":
    ...
    computed = imread("/Users/canfkoyuncu/Downloads/output.tif")
    gold = imread("/Users/canfkoyuncu/Downloads/GT.tif")
    print(computed.shape, np.max(computed), np.unique(computed), gold.shape, np.max(gold), np.unique(gold))

    computed = remove_small_objects(computed>0, min_size=500, connectivity=2, in_place=True)

    computed = preprocessMask(computed)
    gold = preprocessMask(gold)
    print(computed.shape, np.max(computed), computed.dtype, gold.shape, np.max(gold), gold.dtype)
    ...
    print(tp, overseg, underseg, miss, fp)
